...
Breaking Bad & Better Call Saul Edition — v3.1 FIXED

Changes:
  ✅ Removed all GIF dictionary bloat (7 GIFs only)
  ✅ Fixed animation timing and deletion bugs
  ✅ GIFs load immediately without delay
  ✅ All code paths use proper async handling
  ✅ Cache key case sensitivity fixed
  ✅ Watchlist monitor error handling improved
...
# ─────────────────────────────────────────────────────────────
# 3. NARRATIVE 7-GIF SYSTEM - Breaking Bad / Better Call Saul
#    Each GIF tells a story from the TV show
# ─────────────────────────────────────────────────────────────
# 
# 🔍 #1 (scan):       Saul Goodman taking a photo - "Investigation starts"
#    Your link/file is photographed and catalogued
#
# 🧪 #2 (cook):       Walter White cooking meth - "Let me cook"
#    Processing in progress - Walter is analyzing
#
# ✨ #3 (magic):       Saul doing magic/funny stuff - "Dramatic reveal moment"
#    Waiting for results - The suspense before the verdict
#
# 🚨 #4 (suspicious):  Broken plate with missing piece - "Something's off"
#    Suspicious verdict - Something is wrong but not confirmed
#
# ✅ #5 (safe):        Mike saying "All good" - Everything checks out
#    Safe verdict - No threats detected, all clean
#
# ☠️  #6 (danger):      Walter White - "I AM the danger"
#    Dangerous verdict - This IS a real threat
#
# 🎬 #7 (thanks):      Final farewell - "Thank you for visiting our bot"
#    Goodbye message - Safe travels!
# ─────────────────────────────────────────────────────────────
GIFS = {
    "scan":   "https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExdzYxaXRybW4zN3lnMWplajViZ3o4amwwajdxcmY5NzRoaTgxODQzciZlcD12MV9naWZzX3NlYXJjaCZjdD1n/QNGHqtEdHDAcgDPJCj/giphy.gif",
    "cook":   "https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExYzY2ZmhncTdudWllcGZwYjRuendsOTM5MWx3ZWRsOXg3dzB3NnV0NCZlcD12MV9naWZzX3NlYXJjaCZjdD1n/PwLYfy05MBVVm/giphy.gif",
    "magic":  "https://media.giphy.com/media/v1.Y2lkPWVjZjA1ZTQ3czcwOWgwOXNlZ2kwYmFuMmg5NWtzdTJoeXcycGR0MTFsdzlrbzJ0ZyZlcD12MV9naWZzX3NlYXJjaCZjdD1n/3o6YfZMNzUwtMFMXUQ/giphy.gif",
    "suspicious":  "https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExYzY2ZmhncTdudWllcGZwYjRuendsOTM5MWx3ZWRsOXg3dzB3NnV0NCZlcD12MV9naWZzX3NlYXJjaCZjdD1n/2lu3fIfUbfTStOIXx1/giphy.gif",
    "safe":   "https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExZjIzNGRrODMyNjZrMGFyb3M3azRnbWhjM2FvaTFwZXZzY2hnNzVsciZlcD12MV9naWZzX3NlYXJjaCZjdD1n/hSvcVEERKgn0z1XLHV/giphy.gif",
    "danger": "https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExY2RmaXN4MXg5dDN6aDhpOWRmZXVxNGt1cHl0eTNvYmhxNjgyeW0wYyZlcD12MV9naWZzX3NlYXJjaCZjdD1n/3ohc11UljvpPKWeNva/giphy.gif",
    "thanks": "https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExaHlvYXBuMnFzOG51ZDl0Z3VoenVoeWwzajBlZjI5bWVtN2p5M2YxbSZlcD12MV9naWZzX3NlYXJjaCZjdD1n/KVndaGFEqsOvN1l7Xw/giphy.gif",
}


# ── Character-voiced messages — large pool, randomly picked ───
PREP_MSGS = [
    "👔 *Better Call Saul — scanning your case...*",
    "📋 *Saul Goodman is reviewing the file...*",
    "🏛️ *It's all good, man! Analyzing now...*",
    "💼 *Jimmy McGill preparing evidence...*",
    "📞 *Saul's got the lab ready for this.*",
    "⚖️ *Kim Wexler is on it. Quietly. Competently.*",
    "😌 *Huell is standing by. As always.*",
    "🚗 *Nacho is doing recon. Stay calm.*",
]

COOK_MSGS = [
    "🥼 *Walter just put on his hazmat suit.*",
    "⚗️ *The Blue Sky analysis is in progress.*",
    ...
    "🧪 *Walter is calculating the yield.*",
    "☢️ *Heisenberg is in the zone.*",
    "🧑‍🔬 *Jesse: Yeah, science bitch!*",
    "🔥 *Jesse fired up the burners.*",
    "🎵 *Gale is humming while running analysis...*",
]
...
SAFE_INTROS = [
    ...
    "✅ *Kim reviewed it. Her verdict: clean.*",
...
DANGER_INTROS = [
    ...
    "☣️ *Tread lightly. This is cooking trouble.*",
    "☣️ *Gus Fring smiled at this. That means run.*",
    "☣️ *Gus: Delete this immediately.*",
    "☣️ *Lalo Salamanca is smiling at you. Very bad.*",
]
...
SUS_INTROS = [
    ...
    "👀 *Mike sat across watching this all night.*",
    "👀 *Hector is ringing his bell. Be careful.*",
    "👀 *Nacho sniffed this out. Not right here.*",
]
...
def build_result(stats: dict, attrs: dict, kind: str) -> tuple:
    ...
    elif s > 0: verdict, gif_key, intro = "SUSPICIOUS", "suspicious",  pick(SUS_INTROS)
...
# ─────────────────────────────────────────────────────────────
# 11. ANIMATION HELPERS (FIXED - NO DELAYS)
# ─────────────────────────────────────────────────────────────
async def _del(msg):
    """Safely delete a message without blocking."""
    try:
        await msg.delete()
    except Exception:
        pass
...
async def _send_result(
    ...
    """Send final result with magic gif, then thanks gif."""
    # First: Magic GIF with result
    ...
    await update.message.reply_animation(
        GIFS["thanks"],
        caption="✨ *Thank you for using the bot. Stay safe out there.*",
        parse_mode=ParseMode.MARKDOWN,
    )
...
async def scan_hash(update: Update, value: str, htype: str):
    ...
    # FIX: Use lowercase for cache consistency
    ...
    # Scan GIF (quick)
    scan_msg = await update.message.reply_animation(
        GIFS["scan"],
        caption=pick(PREP_MSGS),
        parse_mode=ParseMode.MARKDOWN,
    )
    ...
    try:
        ...
        await _del(cook_msg)
        await update.message.reply_text(
            "❌ Something went wrong. Even Saul cannot fix this right now.",
            parse_mode=ParseMode.MARKDOWN,
        )
...
async def scan_ip(update: Update, value: str, bypass_cache: bool = False):
    ...
    scan_msg = await update.message.reply_animation(
        GIFS["scan"],
        caption=f"🕵️ *Mike is running surveillance on* `{value}`*...*",
        parse_mode=ParseMode.MARKDOWN,
    )
    ...
    cook_msg = await update.message.reply_animation(
        GIFS["cook"],
        caption=f"🔬 *Mike does not miss. Cross-checking* `{value}`*...*",
        parse_mode=ParseMode.MARKDOWN,
    )
...
async def scan_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    scan_msg = await update.message.reply_animation(
        GIFS["scan"],
        caption=f"📦 *Jesse, start cooking — we got a file.*\n`{safe_name}`",
        parse_mode=ParseMode.MARKDOWN,
    )
    ...
    cook_msg = await update.message.reply_animation(
        GIFS["cook"],
        caption=f"🔬 *Scanning with 70+ AV engines...*\n`{safe_name}`",
        parse_mode=ParseMode.MARKDOWN,
    )
...
def main():
    print("╔══════════════════════════════════════════════╗")
    print("║  🦠  VirusTotal Bot — BB + BCS Edition  v3.1║")
    print("║       I am the one who scans.               ║")
    print("║    ✅ All GIFs removed & fixed              ║")
    print("╚══════════════════════════════════════════════╝\n")
